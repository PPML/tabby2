...
scan_for_available_geographies <- function(geography_ids) {
  # available_geographies <- MITUS::load_optim()
    ...
    loc_files <- list.files(system.file(loc, package='MITUS'), full.names = TRUE)

	  all(c(
      any(grepl(paste0(loc, "_results_1.rda"), loc_files)),
		  any(grepl(paste0(loc, "_Optim_all|opt_all"), loc_files)),
		  any(grepl('bg_restab2.rds', loc_files)),
		  any(grepl('sm_restab2.rds', loc_files))
		))
	})])
}
