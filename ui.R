...
sidebar <- dashboardSidebar(
  sidebarMenu(id = 'sidebar',
    ...
    menuItem(
      ...
      menuItem(tabnames[[14]], tabName = names(tabnames)[[14]]), # input costs
      menuItem(tabnames[[15]], tabName = names(tabnames)[[15]]), # costs and outcomes summary
      menuItem(tabnames[[16]], tabName = names(tabnames)[[16]]) # cost comparison
    ),
    ...
    if (
      ...
      ) { menuItem(tabnames[[17]], tabName = names(tabnames)[[17]]) # debug printouts
    } else { NULL },
